...
- [ ] **No-parentheses rollout to the other 54 courses** — `spa_for_eng` pilot closed-loop verified (8/9 hits were a stale-duplicate-JSON sync bug, not fresh content work; 1 to owner decision list). Template + a cheap pre-check that may shrink the true remaining effort below the raw 7,953-row count. → `docs/course-optimization/parentheses-pilot-spa.md`
...
- [x] @claude-local 07-04 — **No-parentheses pilot on `spa_for_eng` closed-loop verified** — 9 hits found, 8 fixed (stale-duplicate `components` JSON sync, no policy call), 1 to owner decision list (gustar-construction gloss, needs a bundled M-LEGO). Audio staged, none generated. Follow-on: "No-parentheses rollout to the other 54 courses" (To build). → `docs/course-optimization/parentheses-pilot-spa.md`
- [x] @claude-local 07-04 — **ZUT rescoped for component rows (Tom's ruling)** — `checkPhraseZUT` + the audit now exempt `phrase_role:'component'` rows from the known-side check but require target-membership (target must be a substring of its own seed's target sentence). Re-audit: fra_for_eng bidirectional 340→110 strict; membership failures 72 (fra) / 76 (spa), mostly-orphan on manual sample — flagged, not fixed. Tests updated (40/40 passing). Follow-on: fix sweep for the membership failures (separate step) + triage the stale-orphan-row hypothesis. → `docs/course-optimization/zut-rescope-component-rows-2026-07-04.md`
