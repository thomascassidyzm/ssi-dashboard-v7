...
- [x] @claude-local 06-15 — **GATE-AND-FIX TEMPLATE TRAVELS TO FRENCH (overnight travel test).** Full lint on `fra_for_eng` (1652 legos, old-way): 5,758 raw flags. English known-side scaffold + 7 tense constructions reused **verbatim** (known=eng). KEY FINDING: the gates encode Chinese-**analytic** typology assumptions — ZUT over-flags inflected targets (**65% of French ZUT = morphology**: clitics/conjugation/gender; only 523 true collisions); known-side over-flags without the target construction layer (**89% = construction-licensing gaps**, collapse once defined). Frame-coverage made **script-aware** (`validation.cjs`, was CJK-only). First-pass French contract committed. **Full brief + prioritised next actions: `docs/course-optimization/MORNING-2026-06-16.md`.**
- [ ] **[#1 NEXT] Target-morphology-aware ZUT gate** — compare target lemmas / per-pair "agreement-exempt" flag, so conjugation/gender/clitics stop reading as ZUT collisions. **Blocks every inflected course (most of the BIG-10).** Lean: per-pair exempt-flag (BSC). → `MORNING-2026-06-16.md` §Next Actions
- [ ] **[#2] Author ~7 zho tense demos + wire `knownTenseConstructions` into the gate** (design in MORNING brief; retire blanket `stemStrip`).
- [ ] **[#3] Ratify French contract from the flags** — define constructions (going-to/`ne…pas`/question-forms/relative/clitics), re-lint to confirm known-side flags collapse.
