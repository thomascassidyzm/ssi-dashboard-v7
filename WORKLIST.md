...
- [!] @tom 06-14 — **Methodology encoding → main** (4 gates: phrase-ZUT on the `/seed/complete` golden path, metadata-gloss, frame-coverage, known-side; + prose + pair-contract template). Pushed to `origin/methodology-encoding`, **pending Tom's morning review** before merge. → branch `methodology-encoding`
- [ ] **zho v2 generation run** using the encoded prompt + gates (300-seed MVP → 668-seed full). The first regeneration that proves the controlled experiment. → `docs/course-optimization/WORKLIST.md`
