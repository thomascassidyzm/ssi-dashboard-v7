...
- [~] @claude-local 07-15 **VARIATION / DIALECT COURSES — expand + fix** (new project, 2026-07-15). Main nine `X_for_eng` are done; now do the variants (deu_at, ara_lb / Syrian / Palestinian, Swiss German, EU/BR Portuguese, regional Spanish, …). **Runbook + full what/how record of the main-course remediation = `docs/course-optimization/main-courses-remediation-playbook.md` (§16 = how to apply to variants).** Method: diff each variant vs its base → run the playbook checklist on the variant-specific delta (register/orthography/lexis/gender) → gender-prep (prompt now fixed for speaker-vs-referent) + Haiku scan → structural edits last → refresh `course_round_index` → audio folds into the full regen.
