 ...
 * - Run phase7-compile-manifest-v2.cjs script (deterministic compilation)
 * - Submit final manifest via POST /api/phase7/:courseCode/submit
 *
...
app.post('/start', async (req, res) => {
  ...
  // Run the v2 compilation script directly (no agent needed - this is deterministic)
  const scriptPath = path.join(__dirname, '../../../scripts/phase7-compile-manifest-v2.cjs');
  ...
  res.json({
    ...
    jobId: `phase7-${courseCode}-${Date.now()}`,
