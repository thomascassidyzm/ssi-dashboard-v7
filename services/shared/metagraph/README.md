...
- **Specification:** `docs/pods/shape-graph-2026-08-30.md` — derived and audited against all
  231 rows of `canonical_pod_scenarios` where `pod_slug = 'pod-1'`. If anything here disagrees
...
mg.coverage('pod-1');       // { traversed, revisited, never, visitCounts }
mg.branches('pod-1');       // the branch points a linear format cannot say
