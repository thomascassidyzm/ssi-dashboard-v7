...
async function generatePresentationAudio(sample, manifest, voiceAssignments, options = {}) {
  ...
  // Compute tempDir after courseCode is destructured (JS default evaluation quirk)
  const tempDir = options.tempDir || (courseCode
    ? path.join(__dirname, '../temp', courseCode, 'presentations', 'natural_processed')
    : path.join(__dirname, '../temp/audio/presentations'));
  ...
  try {
    ...
    if (segmentCache && segmentCache.has(sourcePart)) {
      const cachedPath = segmentCache.get(sourcePart);
      // Wait for file to exist (handles race condition with parallel segment generation)
      const fileExists = await waitForFile(cachedPath, 15, 500); // 15 retries, 500ms each = 7.5s max wait
      if (fileExists) {
        console.log(`  [DEBUG] Using cached source TTS: "${sourcePart.substring(0, 60)}..."`);
        await fs.copyFile(cachedPath, sourceAudioPath);
      } else {
        console.warn(`  [DEBUG] Cache file not found after waiting, regenerating: ${cachedPath}`);
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        if (segment.type === 'text') {
          ...
          if (segmentCache && segmentCache.has(segment.content)) {
            const cachedPath = segmentCache.get(segment.content);
            // Wait for file to exist (handles race condition with parallel segment generation)
            const fileExists = await waitForFile(cachedPath, 15, 500);
            if (fileExists) {
              await fs.copyFile(cachedPath, segmentPath);
            } else {
              // Fall through to generate fresh
              await elevenlabsService.generateAudioWithRetry(
                segment.content,
 ...
 * Prevents race condition when segment cache is being populated concurrently
 *
