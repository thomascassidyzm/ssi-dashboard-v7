...
function zutErrorBody(collisions) {
  return {
    ...
    hint: 'Same known text cannot map to a different target than the course already teaches. '
...
// DIFFERENT row.
async function checkEditedPhrase(supabase, courseCode, { table, id, known, target, revertFields }) {
  const collisions = await checkPhraseZUT(supabase, courseCode, [{ known, target }], null);
