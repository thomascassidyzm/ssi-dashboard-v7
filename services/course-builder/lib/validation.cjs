...
async function loadGenderVariantLicence(supabase, courseCode) {
  const hit = _genderLicenceCache.get(courseCode);
  ...
  try {
    ...
    console.warn(`[gender-licence] could not load for ${courseCode}: ${e.message}`);
  }

  _genderLicenceCache.set(courseCode, { at: Date.now(), pairs });
  return pairs;
}
...
async function checkLegoConflict(supabase, courseCode, knownText, targetText, currentSeedNumber = null) {
  ...
  const licence = await loadGenderVariantLicence(supabase, courseCode);
