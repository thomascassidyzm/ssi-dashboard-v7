  ...
  <div class="bg-canvas/30 rounded-lg p-4 border border-line">
    ...
    <div class="space-y-3">
      ...
      <div>
        ...
        <div class="text-2xl font-bold text-accent-2">{{ estimated }}</div>
      </div>

      <!-- Actual Cost So Far -->
      <div v-if="actual && actual !== '$0.00'">
        <div class="text-xs text-faint mb-1">Actual (so far)</div>
        <div class="actual-cost text-lg font-semibold text-blue-400">{{ actual }}</div>
      </div>
...
<style scoped>
/* Light mode: blue-400 (#60a5fa) only reaches ~2.1:1 on white — fails AA.
   Darken to a blue that passes while keeping the same hue identity.
   Dark mode is untouched (Tailwind text-blue-400 stays). */
:global([data-theme="light"]) .actual-cost {
