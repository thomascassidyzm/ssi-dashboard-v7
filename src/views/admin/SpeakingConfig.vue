    ...
    <div v-else class="rows">
      ...
      <section v-if="labCfg" class="config-row">
        <RowHeader
          ...
          desc="The say-it-yourself gap is one formula: a fixed beat before you start, plus time proportional to how long the answer itself takes to say. Two numbers, and nothing else — how fast the voices play does not change the gap. Set them, then hear them on real sentences. 'Hold after answer' sits AFTER the second target voice rather than before it, so it is not part of the gap."
          :row="rowMap[labMode]"
          :dirty="isDirty(labMode)"
          :saving="savingKey === labMode"
          :error="rowErrors[labMode]"
        ...
        <div class="gap-fields">
          <div v-for="k in GAP_FIELDS" :key="k.key" class="gap-field">
            <label>{{ k.label }} <span class="hint">{{ k.help }}</span></label>
            <input
              type="number"
              :min="k.min" :max="k.max" :step="k.step"
              :value="labCfg[k.key] ?? k.fallback"
              @input="labCfg[k.key] = Number($event.target.value)"
            />
            <span class="gap-unit">{{ k.unit }}</span>
          </div>
        </div>

        <p class="field-note gap-formula">
          Gap = <strong>{{ gapK }}</strong> × how long the answer takes to say
          + <strong>{{ gapReaction }}ms</strong>.
          A one-second answer gets {{ (computePauseFor({ t1ms: 1000, t2ms: 1000 }) / 1000).toFixed(1) }}s,
          a two-second answer {{ (computePauseFor({ t1ms: 2000, t2ms: 2000 }) / 1000).toFixed(1) }}s,
          a three-second answer {{ (computePauseFor({ t1ms: 3000, t2ms: 3000 }) / 1000).toFixed(1) }}s.
          Never shorter than {{ ((labCfg.min_pause_ms ?? 0) / 1000).toFixed(1) }}s
          or longer than {{ ((labCfg.max_pause_ms ?? 0) / 1000).toFixed(1) }}s — those are safety limits, not tuning.
        </p>
        ...
        <div class="lab-hear">
          ...
          <div v-else class="lab-hear-note">Pick a course to hear the configured gap on real sentences.</div>
        </div>
      </section>
...
// The gap model became ONE formula (2026-08-29, Tom's ruling): the gap is
// `pause_k` × the answer's native spoken duration + `pause_reaction_ms`. Rows
// saved before that carry the retired boot/assembly/knee/belt knobs and none of
// them are read any more, so backfill the two new fields to the constants the
// player itself falls back to — that way opening this page shows the gap that
// is ACTUALLY being played, not a 0 that a Save would then make true.
// Easy is the gentler mode, so its k is the larger one.
// Both rows are seeded on creation; this only fires for a row missing a key.
function backfillPauseRow(key, d) {
  const c = d[key]
  if (!c) return
  const isEasy = key === 'easy_mode'
  if (c.pause_k == null) c.pause_k = isEasy ? 3.6 : DEFAULT_PAUSE_K
  if (c.pause_reaction_ms == null) c.pause_reaction_ms = DEFAULT_PAUSE_REACTION_MS
  if (c.min_pause_ms == null) c.min_pause_ms = isEasy ? 2000 : 1000
...
// ============================================================================
// Speaking lab — set a mode's two gap constants and CHECK THEM BY EAR on real
// course sentences. It is deliberately no longer where the algorithm is tuned
// (Tom, 2026-08-29): the gap is one formula, so there is nothing to shape here,
// only two numbers to feel.
// ============================================================================
...
// The two gap constants — the ONLY tunables. Typed, not dragged: these are
// numbers you reason about ("a beat and a bit more than the sentence itself"),
// not a curve you shape.
const GAP_FIELDS = [
  { key: 'pause_k', label: 'Thinking multiplier', min: 0.5, max: 6, step: 0.1, unit: '× the answer', fallback: DEFAULT_PAUSE_K,
    help: 'How much longer than the answer itself the learner gets to build and say it.' },
  { key: 'pause_reaction_ms', label: 'Reaction beat', min: 0, max: 3000, step: 50, unit: 'ms', fallback: DEFAULT_PAUSE_REACTION_MS,
    help: 'The fixed pause before you start — the same whatever the sentence.' },
  { key: 'min_pause_ms', label: 'Never shorter than', min: 0, max: 5000, step: 100, unit: 'ms', fallback: 1000,
    help: 'Safety floor, not tuning — stops a missing duration producing no gap at all.' },
  { key: 'max_pause_ms', label: 'Never longer than', min: 4000, max: 25000, step: 500, unit: 'ms', fallback: 15000,
    help: 'Safety ceiling, not tuning — stops a bad duration producing a 40-second silence.' },
  // Not part of the say-it-yourself gap — this one sits AFTER the answer.
  { key: 'post_voice2_gap_ms', label: 'Hold after answer', min: 0, max: 4000, step: 50, unit: 'ms', fallback: 0,
    help: 'Extra silence held after the second target voice, before the next cycle starts, so the next one does not come straight in over the top. Easy holds 1s; Fast holds none.' },
]
const gapK = computed(() => labCfg.value?.pause_k ?? DEFAULT_PAUSE_K)
const gapReaction = computed(() => labCfg.value?.pause_reaction_ms ?? DEFAULT_PAUSE_REACTION_MS)
    ...
    sampleSentences.value = (seeds || [])
      .map(s => {
        ...
        return {
          ...
          t1ms, t2ms,
          syll: Math.max(1, Math.round(t1ms / MS_PER_SYLLABLE)),
        }
      })
      .filter(s => s.t1ms > 0)
  } catch (e) {
...
// Live gap (ms) for a real sentence under the current unsaved config. Sized off
// the sentence's NATIVE clip durations — playback speed is not an input.
...
// Play one sentence the way the learner hears it: prompt → the configured gap →
// answer, answer. Everything plays at native 1.0× — the belt speed ramp is a
// target-voice effect that no longer touches the gap, so it has no place in a
// surface whose whole job is checking the gap by ear.
...
/* Shared knob header/value pair + range slider (used by the phrase-length and
   review sections; the pause knob grid went with the one-formula gap). */
...
.field-block input[type="range"] { width: 100%; accent-color: #3b82f6; height: 20px; cursor: pointer; }

/* The two gap constants */
.gap-fields { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 0.85rem 1.25rem; margin-bottom: 0.75rem; }
.gap-field { display: grid; grid-template-columns: 1fr auto auto; align-items: center; gap: 0.5rem; }
.gap-field label { grid-column: 1 / -1; font-size: 0.8125rem; color: var(--color-paper, var(--ink)); }
.gap-field label .hint { display: block; font-size: 0.72rem; color: var(--color-paper-dim, var(--faint)); margin-top: 0.15rem; }
.gap-field input {
  width: 110px; background: rgba(0,0,0,0.25); border: 1px solid var(--color-graphite, var(--surface-2));
  border-radius: 6px; color: var(--color-paper, var(--ink)); padding: 0.35rem 0.5rem;
  font-family: var(--font-mono, ui-monospace, Menlo, monospace); font-size: 0.9rem; outline: none;
}
.gap-unit { font-size: 0.75rem; color: var(--color-paper-dim, var(--muted)); }
.gap-formula {
  padding: 0.7rem 0.9rem; border-radius: 8px; line-height: 1.6;
  background: rgba(96, 165, 250, 0.06); border: 1px solid rgba(96, 165, 250, 0.2);
}
.gap-formula strong { font-family: var(--font-mono, ui-monospace, Menlo, monospace); color: #93c5fd; font-weight: 600; }
