...
**C1 and C2 are RULED and have left this list.** *(Propagated 2026-08-28.)* Both were cases where Kai had already ruled and the losing side survived in live files that build agents read at generation time — which is how "compliant but dead" phrases got authored. The ruled positions live at **P7** (USE phrases: full sentences preferred, not required — standalone-sayable is the test, Kai 2026-08-17) and **K9** (the known side must be grammatically correct and read naturally; a ZUT compromise is a rare per-case exception, not a standing licence, Kai 2026-08-06). **Every live location has been corrected**, rationale paragraphs included: the origin passages in `ralph-methodology.md` (L45–51, L227–247, L278–282, L895, L909–919); the build-agent prompt text in `services/course-builder/lib/validation.cjs`, `lib/build-escalation.cjs`, `routes/course-data.cjs` (the resume payload), `routes/v2.cjs`, `services/shared/spawn-course-builder.cjs`, `services/briefs/build-team-creator.cjs`, `services/briefs/backfill-phrases.cjs` and `services/phases/phase1-translation/PROMPT.md`; the skills `calibrate.md`, `checkpoint-qa.md`, `eng-for-jpn-build.md`, `course-resume.md`, `phrase-monitor.md`, `ssi-phrase-variety.md`, `ssi-translation-methodology.md`; `tools/explainer/rulings/docs/glossary.md`; and in the retired docs tree `fix-agent-rules.md` (both the agent block and the "practical reading" reconciliation, now withdrawn) and `course-optimization/HANDOFF-kai-eng-for-x.md`. Dated forensic reports that *quote* the old wording as evidence were left as historical record, one of them annotated with the correction.
...
**K9 — English must be grammatical AND natural; a ZUT compromise is a rare justified exception, not a licence.** · **HARD (grammar), near-HARD (naturalness)** · Kai, 2026-08-06, explicitly settling a documented contradiction between two methodology docs: "It shouldn't sound very weird, especially when English is the target language. It should be grammatically correct — sometimes we might need to pick something slightly suboptimal for ZUT reasons, but usually that's not necessary." · **The bar applies to whatever language the learner already knows, not only English** · If a prompt that tiles reads badly, rewrite the prompt or redraw the LEGO — do not ship the awkward line · **Propagated 2026-08-28: the overruled "slightly stilted but tileable is correct" text has been corrected everywhere it was live, rationale paragraphs included — clash C2 is closed.**
