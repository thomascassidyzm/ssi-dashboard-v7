...
**Human recording is a separate, newer discipline and it is being taken seriously.** `recording-pools.cjs` keeps two intentionally separate pools — Pool A is isolated single-unit reads that are never spliced, Pool B is slow phrase reads and is the only source a splice may draw from (Kai's ruling, 21 August). `recording-speech-gate.cjs` was added on 23 August after Catrin's Welsh takes passed every level-based gate while containing a sheep and room tone: it checks for an actual voiced read rather than just signal above a threshold. Both are recent, both were written from a named incident, and both are the pattern the rest of the estate should copy — a gate that encodes what went wrong, at the one point everything passes through.
