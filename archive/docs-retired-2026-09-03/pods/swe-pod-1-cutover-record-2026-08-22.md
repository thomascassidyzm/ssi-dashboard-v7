# Swedish pods — the switchover, done
...
**Swedish (`swe_for_eng`) listening exercises are replaced.** Learners now get the 231-sentence
...
Two xAI target voices: **Oscar** (`4c7f16ff`) and **Alice** (`3b312632`) — per the T-21 casting
ruling of 2026-08-17 (`docs/pods/t21-casting-rulings-2026-08-17.md`). Bare voice ids, not
`ro-XX-NameNeural`-style Azure ids — checked explicitly for the bare-vs-`xai_`-prefixed duality
(the two forms are the same voice; naive SQL string comparison undercounts by ~14%, per house
memory). Cast-check tool normalizes this: **231/231 linked target clips, 100% on Oscar+Alice, 0
off-cast.**
...
1. **Blank-row cut**: the staged pod's row `SC15-S012` (Narrator, empty text on both sides, no
   ...
   Log: `docs/pods/swe_for_eng-pod-0-unrecorded-blank-row-applied-log.json`.
2. **Proofread-draft waiver**: 115 `target_text_draft` flags cleared (Tom's ruling 2026-08-22 —
   the human-proofread gate on machine-drafted target text is waived estate-wide).
   Log: `docs/pods/swe_for_eng-pod-0-unrecorded-proofread-waiver-applied-log.json`.
3. **Off-cast audit**: on-cast check confirms 231/231 linked target clips on Oscar+Alice, 0
   off-cast. Log: `docs/pods/swe_for_eng-pod-0-unrecorded-off-cast-unlink-dryrun-log.json`.
...
second in the fixed order (after Romanian): **229 generated, 2 reused, 0 failed. Veracity: 12/12
sampled clips checked and passed, 0 quarantined.** Elapsed ~26.5 min.
...
1. **Full probe-all audio verification** (`verify-pod-audio.cjs --probe-all`): 231 rows, 462/462
   ...
   tracks. Committed: `docs/pods/pod-audio-verify-swe_for_eng_pod-0-unrecorded.json`.
2. **On-cast check**: 231 linked target clips, 231 on-cast, 0 off-cast — exactly the two
   normalized voice ids `3b312632` (Alice) + `4c7f16ff` (Oscar).
3. **Prospective migration log**, committed before the real course was touched (commit
   `461e2d623`): `docs/pods/swe-pod0-switchover-prospective-2026-08-22.json` — 108 content
   survivors, 0 ambiguous, carry 58 / drop 21, 23 mis-credits prevented.
4. **Full rehearsal on a throwaway clone** (`rehearse-switchover.cjs --scratch=zzz_rehearsal_swe`)
   — forward and rollback both landed, pods restored to their original slugs and counts, zero
   orphaned progress. Forward pass matched the forecast exactly (58 carried / 21 dropped). Scratch
   cleaned up afterward.
5. **The flip** (`pod-switchover.cjs --apply`): archived 142 → `pod-0-retired-2026-08-22`,
   promoted 231 → `pod-1`. Learner progress: **58 carried, 21 dropped** — exactly matching the
   forecast.
6. **Independent database re-read**: `pod-1` — 231 sentences, 231/231 both audio tracks, titled
   "Swedish Listening Pods — Pod 1"; `pod-0-retired-2026-08-22` — 142 sentences, 142/142/142,
   archived intact; `learner_pod_state` for `swe_for_eng` — 58 rows / 1,810 exposures, matching
...
| | Forecast | Actual |
|---|---:|---:|
| Records carried | 58 | **58** |
| Exposures carried | 1,810 | **1,810** |
| Records dropped | 21 | **21** |
| Mis-credits prevented | 23 | **23** |

Landed exactly on forecast.

---

## Honest note: 53 pre-existing `:sN` orphans, zero new ones

`learner_pod_state` rows for `swe_for_eng` with no matching `listening_pod_sentences.id`: **53
...
| Check | Result |
|---|---|
| `listening_pods` for `swe_for_eng` | Exactly two rows: `pod-1` (live) and
`pod-0-retired-2026-08-22` (archived). No `pod-0` remains to be mistakenly served. |
| First sentence, `swe_for_eng:pod-1:SC01-S001` | target "Godmorgon, Sarah!" / known "Good
...
```
node tools/pods/pod-switchover.cjs --course=swe_for_eng --stamp=2026-08-22 --promote-to=pod-1 --rollback --apply
```
...
Nothing new. The Swedish cast was already ruled (T-21, 2026-08-17); this course simply carried it
out.

---

*Protocol: `docs/pods/pod-migration-protocol.md` (plate A-111, adopted 2026-08-16). Prospective
migration log: `docs/pods/swe-pod0-switchover-prospective-2026-08-22.json`. Audio verification:
`docs/pods/pod-audio-verify-swe_for_eng_pod-0-unrecorded.json`.*
