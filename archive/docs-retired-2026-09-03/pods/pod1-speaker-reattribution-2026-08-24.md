...
- **The script viewer** shows the two lines as red `uncast-character` fails, plus 13 amber
  ...
  two characters speak, one has no voice. Confirmed against the **running** service, not
  just the code: `GET localhost:3470/api/pod-scripts/ita_for_eng` returns 29 violations
  including both uncast fails.
...
## One explicit gap: the flagger change is merged but NOT deployed

The services run from the `ssi-dashboard-v7-clean-prod` checkout, which sits on `main` at
`57ed8fd17` — behind these commits. I checked what the running service actually serves
rather than assuming:

- **The attribution IS live.** The running API returns `Staff` and `Interlocutor` and both
  uncast fails. That is data, so it needed no deploy.
- **The addressing rule is NOT live.** The same response returns `course: undefined` and the
  old un-prefixed messages, because the running process is on older code.

So findings coming out of the live viewer today are still in the shape the rule bans. The
fix is a pull and restart of the prod checkout, which I have not done — restarting the
production services was not in this brief and is not mine to do unasked.

---

**Landing line.** Four commits are on `main`: `47d32f00a` (attribution), `49b0fbf8f`
(flagger rule), `05e411a66` (this report) and the commit carrying this line. They were
**merged to `main`**, which is where they live; branch
`fix/pod1-speaker-attribution-2026-08-24` holds the same work on its original lineage and
was deliberately left un-force-pushed. **Not deployed:** the `ssi-dashboard-v7-clean-prod`
checkout that runs the services is still at `57ed8fd17`, and I verified live on
`localhost:3470` that it serves the new attribution (data) but the old, un-addressed
finding text (code).
