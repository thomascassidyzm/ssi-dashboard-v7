# Mexican Spanish pods — the switchover, done
...
**Mexican Spanish listening exercises are replaced.** Learners now get the 231-sentence,
22-scene pod in place of the 142-sentence pod they had before, under the same 1-based `pod-1`
convention already carried by Croatian, Italian, French, Chinese, Japanese, Arabic and (parent)
Spanish.

**Cast**: Luciano (male, Azure `es-MX-LucianoNeural`) and Carlota (female, Azure
`es-MX-CarlotaNeural`) — both **native** Azure Mexican Spanish voices, no xAI multilingual fallback
involved. Tom's ruling of 2026-08-18.

This is the one regional **variant** in the batch. It kept its own native `es-MX` cast and
inherited nothing from the Spanish parent course's voices or vocabulary — Tom's standing position
is that variants are their own languages, with their own voices and their own vocabulary pass,
never re-voiced parent text.

Both of Mexican Spanish's pre-existing target clips were **off-cast** — the previous render was in
a voice Tom never picked — so this was a full re-render, not a top-up. This was caught only by the
on-cast check: the readiness gate counts sentences with no audio at all, and is blind to audio that
exists but is in an uncast voice, so this course would otherwise have shipped reading "ready" in an
unapproved voice.
...
5-clip sample clean first, then bulk. 0 failed.
...
(`docs/pods/spa_mx-pod0-switchover-prospective-2026-08-22.json`, read directly for this record)
shows 109 content survivors between the old 142-sentence canon and the new 231-sentence one, zero
...
3. **Independent database re-read**, this job's own query, confirms: `pod-1` — 231 sentences,
   titled "Mexican Spanish Listening Pods — Pod 1"; `pod-0-retired-2026-08-22` — 142 sentences;
   both known and target audio 231/231 linked on `pod-1`.
4. **Full-audio probe**: 461 of 461 distinct audio ids resolved (461 not 462 because two rows
   legitimately share one clip), HEAD ok=231 missing=0 on both tracks, ffprobe ok=231 bad=0 on
   both tracks.
...
6. **End-of-clip click check** — three fresh clips, tail-400ms peaks -13.5, -12.0 and -23.4 dB,
   speech running to the end, no isolated impulse. No click. (Tom abolished click/tail repair on
   2026-08-17 — report only, never patch.)
...
Mexican Spanish doesn't have a login-free learner path to click through by hand in this job's time
box, so verification was run the way the app itself runs it: the exact Supabase REST query the
deployed app's `servedPod` resolver issues (`listening_pods` filtered to `pod_type=core`,
preferring `pod-1` over `pod-0`), replayed against **`https://staging.saysomethingin.app`**'s and
...
`GET /api/pods/spa_mx_for_eng/pod-1` call the Popty admin page itself makes. **Every check below
passed on both.**
...
| Mexican Spanish resolves to | **`pod-1`** | **`pod-1`** |
| Mexican Spanish sentence / scene count | **231 sentences, 22 scenes** | **231 sentences, 22 scenes** |
| Mexican Spanish audio linked | **231/231 target, 231/231 known** | **231/231 target, 231/231 known** |
| First Mexican Spanish clip via the live audio proxy | **200, `audio/mpeg`, 25,920 bytes** | **200, `audio/mpeg`, 25,920 bytes** (same clip, same DB) |
...
Supabase database, so "Mexican Spanish resolves correctly" is necessarily the same fact checked
twice through two different JS bundles, not two different facts. What staging genuinely adds is
...
`/production/spa_mx_for_eng/pods/pod-1` was not driven directly — Popty is behind an email
...
direct database read and by the same API the page calls, run locally against production data
(`GET /api/pods/spa_mx_for_eng/pod-1` → slug `pod-1`, title "Mexican Spanish Listening Pods — Pod
1", 231 sentences). The old slug, `/production/spa_mx_for_eng/pods/pod-0-unrecorded`, now 404s as
documented — confirmed against the same API
(`GET /api/pods/spa_mx_for_eng/pod-0-unrecorded` → 404).
...
```
node tools/pods/pod-switchover.cjs --course=spa_mx_for_eng --stamp=2026-08-22 --promote-to=pod-1 --rollback --apply
```
...
migration log: `docs/pods/spa_mx-pod0-switchover-prospective-2026-08-22.json`.*
