<!-- Course leader / editor persona. Mechanism only. The drift gate requires
every surface an editor can stand on — including the docs tabs — to be named
here. Checking has no role of its own yet, so the checker's view lives in the
checking section below until auth grows one. -->
