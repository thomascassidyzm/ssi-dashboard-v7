...
**Key insight:** If one side is a single word, it's automatically A-type because you can't split that word.

```
"voy" → "I go/I'm going"       A-type (1 word Spanish)
"querer" → "want to"           A-type (1 word Spanish)
"contigo" → "with you"         A-type (1 word Spanish)
```
