...
function itemToCycle(item) {
  // Review items carry their voiced content in basket[0] rather than at the
  // item's own top level (the generator moved to this basket-of-USE-phrases
  // shape for the learner's spaced-review draw after this schema was fixed).
  const src = (item.type === 'review' && item.basket && item.basket[0]) ? item.basket[0] : item
  const base = {
    type: item.type,
    round_number: item.roundNumber,
    lego_id: item.legoId,
    known_text: src.known_text,
    target_text: src.target_text,
    target1_audio_id: src.target1_audio_uuid || null,
    target2_audio_id: src.target2_audio_uuid || null,
  }
  if (item.type === 'intro') {
    base.presentation_audio_id = item.presentation_audio ? item.presentation_audio.id : null
  } else {
    base.known_audio_id = src.known_audio_uuid || null
  }
  return base
}
