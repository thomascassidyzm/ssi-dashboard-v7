...
describe('parsePod', () => {
  it('defaults to pod-1, the core listening pod, when no --pod flag is present', () => {
    expect(parsePod(['node', 'script.cjs', 'hrv_for_eng'])).toBe('pod-1')
  })

  it('reads an explicit --pod=pod-N flag', () => {
    expect(parsePod(['node', 'script.cjs', 'hrv_for_eng', '--pod=method-pod'])).toBe('method-pod')
  })
