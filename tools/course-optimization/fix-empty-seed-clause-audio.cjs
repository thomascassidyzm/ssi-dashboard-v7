 ...
 * output, or FAIL_STREAK consecutive rows do, the run aborts non-zero.
 *
...
async function main() {
  ...
  let ok = 0, failed = 0, skippedHuman = 0, streak = 0, first = true, aborted = null
  const t0 = Date.now()
  ...
  const renderOne = async (row) => {
    ...
    doneStream.write(JSON.stringify({ id: row.id, ok: false, reason, text: row.after }) + '\n')
    console.error(`[${course}] FAIL ${row.id}: ${reason}`)

    // Fail loudly rather than burn thousands of calls into a dead unit.
    if (first) aborted = `first row produced no output: ${reason}`
    else if (streak >= FAIL_STREAK) aborted = `${FAIL_STREAK} consecutive failures, last: ${reason}`
    first = false
  }
  ...
  console.log(`[${course}] DONE: ${ok} rendered, ${failed} failed, ${skippedHuman} human-skipped in ${mins} min`)
