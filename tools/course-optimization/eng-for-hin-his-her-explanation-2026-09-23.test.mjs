 ...
 *     terms, no brackets, quotes its chunk and its own seed, ends in the introduction.
...
const INTRO = t.planIntro('उसका नाम', SEEDS[0].known_text)
...
describe('where: the first his/her LEGO, measured', () => {
  ...
  it('refuses when the seed does not contain the chunk (Frame B would misquote)', () => {
    const seeds = SEEDS.map((s) => (s.seed_number === 20 ? { ...s, known_text: 'कुछ और।' } : s))
    expect(() => t.planLine({ legos: LEGOS, seeds })).toThrow(/does not contain/)
  })
})

describe("what: Kai's rails on the line", () => {
  const plan = t.planLine({ legos: LEGOS, seeds: SEEDS })
  it('is Hindi only, no grammar terms, no brackets, quotes chunk and seed, ends in the intro', () => {
    expect(t.lineProblems(plan.text, { chunk: 'उसका नाम', seedKnown: SEEDS[0].known_text })).toEqual([])
  ...
  it('the rails catch what they are for', () => {
    const p = (s) => t.lineProblems(s, { chunk: 'उसका नाम', seedKnown: SEEDS[0].known_text })
    ...
    expect(p(`${t.EXPLANATION} अंग्रेज़ी में — 'उसका नाम' — में :`)).toContainEqual(expect.stringMatching(/own seed/))
  })
})
