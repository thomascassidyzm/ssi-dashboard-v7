...
**C14 is RULED and has left this list.** *(Kai, 2026-09-04.)* A component is **exempt from the collision test by construction** — "they never play again after that first introduction so there's no risk of a clash" — so `glossary.md:54` is right and `phase1-translation/PROMPT.md:229` is the losing side. The rule now lives at **L23**.

> **One question, deliberately not answered here.** Tom ruled on C14 on 2026-08-28 (unmerged branch `docs/canon-c14-resolved-2026-08-28`) that attested and deterministic are two separate tests and a component must pass both. Kai's 2026-09-04 words are about the *clash* half and post-date it. **Do these two rulings agree?** They have never been put side by side, and this document does not reconcile them.
...
**L23 — Components are held to a LOOSER standard than LEGOs, are scanned separately, and cannot fail the collision test.** · **HARD** · **Kai, 2026-09-04, closing clashes C13 and C14** · Judging a component by the LEGO standard generates a queue of non-defects; treating it as a LEGO for collision purposes blocks a construction that by design is never prompted again · A component may read oddly on its own without that being a defect.
...
> **Not settled here, and deliberately so.** Tom ruled on 2026-08-28 (unmerged branch `docs/canon-c14-resolved-2026-08-28`) that a component IS available vocabulary and is NOT exempt from *determinism* — attested and deterministic being two separate tests. Kai's words above are about the *clash* half and post-date that by a week. **On one reading they do not conflict; they have never been put side by side, and this entry does not reconcile them.** Ask Kai.
...
**K2 — ZUT is the ZERO UNCERTAINTY TEST: prompted in the known language, the learner has no uncertainty about what they are meant to say.** · **HARD — the top rail** · **Kai, 2026-08-27 and 2026-09-02, closing clashes C9 and C10**; `ralph-methodology.md:39`; `CLAUDE.md` · A learner with a deterministic production function interacts fearlessly; one juggling natural variants freezes.
