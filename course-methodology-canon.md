...
18. **The standing rule on seed text is not written down here as an entry.** Kai's L25 addendum leans on it — a seed is not reworded because a better phrasing exists, only because something is actually wrong — and this canon holds the pieces of it in four places (**S6** the canonical seed, **S7** never change canonical meaning, **R0.3** rewrites are reported not applied, and "Editing translated seed text in a course he owns" under what is Kai's call) plus `ralph-methodology.md:399`'s "you cannot quietly reword the seed", **but nowhere states it as one rule with the five fields.** It is cited in **L25** as the thing that rung 2 does not loosen. Writing it as its own entry needs Kai's own words for its scope, which this job did not have, so it is reported rather than drafted.
