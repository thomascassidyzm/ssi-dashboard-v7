-- 20260913_retire_pod_0.ROLLBACK.sql — restores the serving_pod view and
-- estate_map() exactly as dumped from production on 2026-09-13 before
-- 20260913_retire_pod_0.sql ran. Data renames are undone with
-- tools/pods/retire-pod-slug.cjs --allow-backwards, per course, not here.

CREATE OR REPLACE VIEW public.serving_pod AS  SELECT DISTINCT ON (course_code) course_code,
    slug,
    id AS pod_id
