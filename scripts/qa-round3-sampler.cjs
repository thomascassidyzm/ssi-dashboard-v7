#!/usr/bin/env node
/**
 * Round 3 QA Sampler - USE phrases from FIRST 50 SEEDS only
 * Tests "starting well" quality - learner's first impressions
 */
...
async function sampleCourse(courseCode) {
  ...
  // Shuffle and take up to 50 (may have fewer if first 50 seeds don't have enough USE phrases)
  ...
  const output = {
    ...
    round: 3,
    seed_range: '1-50',
    description: 'First 50 seeds only - testing "starting well" quality',
    total_use_phrases_in_range: usePhrases.length,
  ...
  const outPath = path.join(__dirname, 'qa-comparison', courseCode + '-sample-r3.json')
  fs.writeFileSync(outPath, JSON.stringify(output, null, 2))
  return { course: courseCode, sampled: sample.length, available: usePhrases.length }
}

async function main() {
  console.log('Generating Round 3 samples (USE phrases from seeds 1-50 only)...\n')

  for (const course of courses) {
    const result = await sampleCourse(course)
    if (result) {
      console.log(result.course.padEnd(18) + ': ' + result.sampled + ' sampled from ' + result.available + ' USE phrases (seeds 1-50)')
    }
  }

  console.log('\nDone! Files saved as *-sample-r3.json')
}
