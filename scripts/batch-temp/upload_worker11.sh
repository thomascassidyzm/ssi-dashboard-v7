...
# Upload Worker 11 Baskets
# Usage: ./upload_worker11.sh

ENDPOINT="https://mirthlessly-nonanesthetized-marilyn.ngrok-free.dev/upload-basket"

echo "🚀 Uploading Worker 11 Baskets"
echo "================================"
echo ""

# S0121L02 - unusual/不寻常
echo "📤 Uploading S0121L02 (unusual/不寻常)..."
...
if [ $? -eq 0 ]; then
  echo "✅ S0121L02 uploaded successfully"
else
  echo "❌ S0121L02 upload failed"
fi

echo ""

# S0121L03 - your/你的
echo "📤 Uploading S0121L03 (your/你的)..."
curl -X POST "$ENDPOINT" \
  -H "Content-Type: application/json" \
  -d '{
    "course": "zho_for_eng",
...
if [ $? -eq 0 ]; then
  echo "✅ S0121L03 uploaded successfully"
else
  echo "❌ S0121L03 upload failed"
fi

echo ""
echo "================================"
echo "✅ Worker 11 resubmitted: 2 LEGOs"
