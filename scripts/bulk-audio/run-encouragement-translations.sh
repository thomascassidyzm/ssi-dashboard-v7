#!/bin/bash
# Translate 96 NEW English encouragement+instruction texts into 35 known languages.
# Spawns 35 parallel claude --print Opus calls. Output: temp/encouragement-migration/output/{lang}.json
...
LANGS="ara aze ben bul ces dan deu ell fil fin fra guj hin hrv ind ita jpn kor lit msa nld pan pol por ron rus sin slk spa swe tam tur ukr urd zho"
...
for lang in $LANGS; do
  echo "[enc-mig] launching $lang"
  run_one "$lang" > "$LOGS/${lang}.stdout" 2>&1 &
  PIDS="$PIDS $!:$lang"
done
echo "[enc-mig] launched 35 jobs at $(date +%H:%M:%S)"

for entry in $PIDS; do
  pid="${entry%%:*}"; lang="${entry##*:}"
  if wait "$pid"; then echo "[enc-mig] $lang -> OK"; else echo "[enc-mig] $lang -> FAIL"; fi
done

echo "[enc-mig] all done in $(($(date +%s) - START_TS))s"
