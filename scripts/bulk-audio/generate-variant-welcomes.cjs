...
/**
 * Generate Welcome Audio for Regional Variants + Missing Courses
 *
 ...
 *
 * Voice_id is set to 'elevenlabs_FVdzAUsp8apoOdc0907A' and origin='tts'
 * to distinguish from real Aran recordings.
 *
 * Usage:
 *   node generate-variant-welcomes.cjs --plan     # Show plan
 *   node generate-variant-welcomes.cjs --execute   # Generate + master + upload + DB
 */
...
// Template: first mention uses full variant name, rest uses base language name
const TEMPLATE = (variantName, baseName) =>
...
// Standard template (no variant distinction needed)
...
const WELCOMES = [
  { course_code: 'spa_mx_for_eng', text: TEMPLATE('Mexican Spanish', 'Spanish'), lang_code: 'en' },
  { course_code: 'por_br_for_eng', text: TEMPLATE('Brazilian Portuguese', 'Portuguese'), lang_code: 'en' },
  { course_code: 'ara_eg_for_eng', text: TEMPLATE('Egyptian Arabic', 'Arabic'), lang_code: 'en' },
  { course_code: 'ara_sy_for_eng', text: TEMPLATE('Syrian Arabic', 'Arabic'), lang_code: 'en' },
  { course_code: 'deu_at_for_eng', text: TEMPLATE('Austrian German', 'German'), lang_code: 'en' },
  { course_code: 'pol_for_eng',    text: STANDARD_TEMPLATE('Polish'), lang_code: 'en' },
  { course_code: 'tur_for_eng',    text: STANDARD_TEMPLATE('Turkish'), lang_code: 'en' },
  { course_code: 'bre_for_eng',    text: STANDARD_TEMPLATE('Breton'), lang_code: 'en' },
];
...
async function main() {
  ...
  if (!isPlan && !isExecute) {
    console.log('Usage:');
    console.log('  node generate-variant-welcomes.cjs --plan     # Show plan');
    console.log('  node generate-variant-welcomes.cjs --execute   # Generate all');
    process.exit(0);
  }
  ...
  console.log(`\n${'='.repeat(65)}`);
  console.log('  VARIANT WELCOME GENERATION PLAN');
