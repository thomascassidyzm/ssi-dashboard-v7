...
async function addUsePhrases() {
  const newPhrases = [
    ...
    // S154L1 - needs 4 more USE (currently has 4)
    {
      ...
      phrases: [
        {known: "do you want to meet at the pub tomorrow evening", target: "willst du dich morgen abend in der kneipe treffen"},
        {known: "do you want to meet before work tomorrow morning", target: "willst du dich morgen früh vor der arbeit treffen"},
        {known: "do you want to meet my new friend this weekend", target: "willst du dich mit meinem neuen freund treffen"},
  ...
  for (const entry of newPhrases) {
    ...
    for (let i = 0; i < entry.phrases.length; i++) {
      ...
      // Check containment
      const targetNorm = p.target.toLowerCase().replace(/\s+/g, ' ');
      const legoNorm = entry.lego_target.toLowerCase().replace(/\s+/g, ' ');
      if (!targetNorm.includes(legoNorm)) {
      ...
      const legoPos = targetNorm.indexOf(legoNorm);
