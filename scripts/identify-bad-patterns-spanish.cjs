...
/**
 * Identify common bad patterns in Spanish baskets
 *
 * Same analysis as Chinese - check S0001-S0100 for mechanical errors
 */
...
const BAD_PATTERNS = [
  ...
  { regex: /\bto speak (often|now|today) (Spanish|English)/i, desc: 'Wrong adverb placement' },

  // "I want to X" where X is not a verb
  { regex: /\bI want to (Spanish|English|some|often|with|now)\b/i, desc: 'I want to + non-verb' },

  // "I'm trying to X" where X is not a verb
  { regex: /\btrying to (some|often|with|now|today|Spanish)\b/i, desc: 'trying to + non-verb' },
...
async function identifyBadPatterns() {
  ...
  console.log('📖 Loading Spanish lego_baskets.json...\n');
  ...
  // Display results
  console.log('🔍 BAD PATTERN ANALYSIS - SPANISH (S0001-S0100)\n');
  console.log('='.repeat(80));

  if (Object.keys(byPattern).length === 0) {
    console.log('\n✅ No bad patterns detected!');
    console.log('   Spanish baskets appear to be high quality (agent-generated)');
  } else {
  ...
  if (issues.length > 0) {
    ...
    console.log('   These baskets appear to be properly agent-generated');
  }
}
