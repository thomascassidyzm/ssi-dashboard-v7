...
/**
 * Phase 5: Detect Gaps in Agent Output Files
 *
 * Detects missing agent output files in a segment
 *
 * Usage: node scripts/phase5-detect-gaps.cjs <course> <segment>
 *
...
if (!courseCode || !segmentNum) {
  console.error('Usage: node scripts/phase5-detect-gaps.cjs <course> <segment>');
  console.error('');
  console.error('Example: node scripts/phase5-detect-gaps.cjs spa_for_eng 1');
  process.exit(1);
}

const segmentId = String(segmentNum).padStart(2, '0');

console.log('🔍 Phase 5: Gap Detection');
...
for (let orch = 1; orch <= orchestratorCount; orch++) {
  ...
  if (!fs.existsSync(orchDir)) {
    ...
    for (let agent = 1; agent <= agentsPerOrch; agent++) {
      ...
      missing.push(`orch_${orchId}/agent_${agentId}_baskets.json`);
    }
    continue;
  }
  ...
  for (let agent = 1; agent <= agentsPerOrch; agent++) {
    const agentId = String(agent).padStart(2, '0');
    const agentFile = path.join(orchDir, `agent_${agentId}_baskets.json`);
    const relativePath = `orch_${orchId}/agent_${agentId}_baskets.json`;

    if (fs.existsSync(agentFile)) {
      found.push(relativePath);
      console.log(`  ✓ agent_${agentId}_baskets.json`);
    } else {
      missing.push(relativePath);
      console.log(`  ✗ agent_${agentId}_baskets.json [MISSING]`);
    }
  }
  console.log('');
}
...
if (missing.length > 0) {
  ...
  console.log('');
  console.log('To regenerate missing agents, use:');
  console.log(`node scripts/phase5-spawn-missing-agent.cjs ${courseCode} ${segmentNum} <orch> <agent>`);
  console.log('');
  process.exit(1);
} else {
  ...
  console.log(`node scripts/phase5-merge-segment.cjs ${courseCode} ${segmentNum}`);
  console.log('');
  process.exit(0);
}
