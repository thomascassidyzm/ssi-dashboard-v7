...
// Fixed phrase generator with correct seed sentences and no violations
function gen(seedData, lego, isLast, wl) {
  ...
  if (sid === 'S0341' && lego.id === 'S0341L01') {
  ...
  } else if (sid === 'S0341' && lego.id === 'S0133L02') {
    p.push([k, t, null, 1]);
  ...
  } else if (sid === 'S0341' && lego.id === 'S0128L03') {
    p.push([k, t, null, 1]);
  ...
  } else if (sid === 'S0341' && lego.id === 'S0341L04') {
    ...
    // Default: proper 2-2-2-4 distribution
...
console.log('=== Fixing Agent 03 Output ===\n');
...
for (const sd of input.seeds) {
  ...
  for (let i = 0; i < sd.legos.length; i++) {
    ...
    console.log(`  ${lg.id}: ${d.really_short_1_2}-${d.quite_short_3}-${d.longer_4_5}-${d.long_6_plus}${isLast?' [FINAL]':''}`);
  }
}

fs.writeFileSync('batch_output/agent_03_baskets.json', JSON.stringify(out, null, 2));
console.log('\n✅ Fixed - now re-validate');
