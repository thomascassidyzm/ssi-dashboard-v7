...
import { loginAsTestUser, pinApiBase, TEST_COURSE, API_BASE } from './helpers.js'
...
test.describe.serial('pod recording — mode chooser, two-voice cast, dialogue recording', () => {
  ...
  test('mode chooser -> Listening Pods opens the cast panel', async ({ page }) => {
    ...
    await expect(page.getByText('Cast — the two voices')).toBeVisible()
  })
  ...
  test('cast exactly two voices (1 male, 1 female) and save', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Work out the parts' }).click()
    await expect(page.getByText(/The parts — review, then Save cast/)).toBeVisible()

    // Two allocation cards, one per voice.
    const cards = page.locator('.cast-row', { has: page.locator('text=/Copy record link|Save cast to get the link/') })
  ...
  test('recorded pod audio: DB rows + storage + UI playback', async ({ page }) => {
    ...
    const podRow = psql(`select id from listening_pods where course_code='${TEST_COURSE}' and slug='pod-0';`)
    expect(podRow).toBeTruthy()

    const withAudio = Number(psql(
      `select count(*) from listening_pod_sentences where pod_id='${podRow}' and target_audio_id is not null;`
    ))
    expect(withAudio).toBeGreaterThan(0)

    const humanRows = Number(psql(
      `select count(*) from course_audio where course_code='${TEST_COURSE}' and origin='human' and role='target1';`
    ))
    expect(humanRows).toBeGreaterThan(0)

    const s3Key = psql(
      `select s3_key from course_audio where course_code='${TEST_COURSE}' and origin='human' and role='target1' limit 1;`
    )
