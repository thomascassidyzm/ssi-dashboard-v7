...
//
// ── ONE CALIBRATION LOOP WAS RUN, AND WHAT IT CHANGED ────────────────────────────────────────────
// The first draft of freeClass was built from the ranked lego+phrase+seed frequency list and then
// tested with `known-side-sweep.cjs eng_for_ara`. Reading the residual, three families of genuine
// function words had been missed and were added: (a) the FEMININE and PLURAL members of the
// preposition+suffix and conjunction+suffix portmanteau sets — I had listed أنه/به/عنه/فيه but not
// أنها/بها/عنها/فيها/أنهم/لكنها/ولكنها, which is an inconsistency in me, not in Arabic (~110 hits);
// (b) أم (21), the alternative-question 'or', distinct from أو; (c) إن (19) and لذا (7). Everything
// else in the residual was morphology or a real finding, and was left alone. Recording this so the
// next author knows the list was tested rather than merely asserted.
module.exports = {
  ...
  glossRules: [
    ...
    { id: 'negationSubstringOverreach', rule: "MEASURED IMPLEMENTATION CAVEAT, not a fact about Arabic. checkKnownSide, given a brief contract with no negationMarkers regex, decides whether a prompt is negated by testing whether the prompt STRING CONTAINS any word in the `negation` list. Arabic negators are one and two letters long (لا، ما، لم، لن), and those sequences occur inside ordinary content words (الاسترخاء contains لا; الكلام contains لا). MEASURED on eng_for_ara's 4,975 drilled phrases (2026-08-17): the substring test calls 3,205 of them (64%) negated, while only 1,459 (29%) actually contain a negator as a TOKEN. So 1,746 positive prompts — 35% of the course — are silently treated as licensing environments. The direction of the error is SAFE (it suppresses NPI findings rather than inventing them) but the magnitude means a low NPI count on an Arabic course is NOT evidence that the NPI rule was checked. Do not report it as one. Fixing this needs a token-level negation test in checkKnownSide, not a change to this file." },
