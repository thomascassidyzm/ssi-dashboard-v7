...
module.exports = {
  ...
  glossRules: [
    ...
    { id: 'negationSubstringOverreach', rule: "MEASURED IMPLEMENTATION CAVEAT, not a fact about Yoruba. checkKnownSide, given a brief contract with no negationMarkers regex, decides whether a prompt is negated by testing whether the prompt STRING CONTAINS any word in the `negation` list. Yoruba negators are one and two characters (ò, kò, kì, má), and those sequences occur inside ordinary words (lóòrèkóòrè contains ò; máa contains má). MEASURED on cym_for_yor's 168 drilled phrases (2026-08-17): the substring test calls 80 of them (48%) negated, while only 8 (5%) actually contain a negator as a TOKEN. So 72 positive prompts — 43% of the course — are silently treated as licensing environments. That is a worse ratio than Arabic's (64% vs 29%), because má is a proper prefix of the future particle máa, which is everywhere. The error direction is SAFE (it suppresses NPI findings rather than inventing them) but a zero NPI count on a Yoruba course is NOT evidence the NPI rule was checked. Do not report it as one. Fixing this needs a token-level negation test in checkKnownSide, not a change to this file." },
