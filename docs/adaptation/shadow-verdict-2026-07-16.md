...
## 5. Re-soak started — 2026-07-16

All four mechanical fixes from §2/§4 have landed (`ssi-learning-app`, `dev` → `staging`, engine still shadow-only on `main`):

1. **Blind sensor** — `evidence.ts`'s aggregator stamped `x = occurredAtMs` (session-milliseconds) on every ring entry; fixed to the sample's ring position (cycle-index), matching what `localDifficulty`/`curvature` already assumed everywhere else. Reproduced the crush with the synthetic-struggle method (ms x-axis → `steady`, fitted acceleration ~1e-10; cycle-index x-axis → `struggling`, sane z ≥ 2) and locked it in as a regression test (`evidence.test.ts`, "regression — blind sensor").
2. **Persistence silently broken** — root cause was NOT RLS or a missing migration (both were already correct and live): `useAdaptationEngine`'s config captured `supabase`/`learnerId` as plain dereferenced values at construction. `LearningPlayer.vue`'s own `onMounted` (a child component) fires before its parent `App.vue`'s `onMounted` even calls `auth.initialize()` — so the captured `learnerId` was frozen pre-auth-resolution for the life of the engine, every session. Fixed to accept `Ref`s (mirroring `useBeltProgress`'s already-proven pattern) and re-read live on every `flush()`/`initialize()`, with a `watch()` that fires the one-time mastery/evidence hydration once the id actually resolves. Failures now call `onPersistenceError` (wired to `logEvent('adaptation_persistence_error', ...)`) instead of a bare `console.warn`.
3. **Config never seeded** — inserted the `adaptation_v2` row into `algorithm_config` (`enabled:true, shadow:true, stage2_enabled:false`, spec's shipping bounds), applied live via psql, `schema.sql` re-snapshotted. Verified readable via `scripts/db-read.mjs adaptation_v2`.
4. **Criticality inversion** — `planRound` only carried the round's own LEGO in `unitOrdinals`; every other unit (including seed-1 LEGOs read in review phase) fell through `isCritical` as "unknown → not critical → deferrable". Fixed by deriving every read unit's ordinal directly from its own id (`getSeedFromLegoId`, the same "seed number is the ordinal proxy" convention the caller already used) — no new plumbing. Regression test proves a seed-1 LEGO's struggle no longer moves the round-budget levers, with a contrast case proving genuinely non-critical units still defer correctly.

All four ship with regression tests; full `@ssi/core` + `player-vue` suites (804 tests) and typecheck green.

**The bar for flipping stays exactly what §4 already said**: not a calendar date, but multiple non-default plans in the shadow log that read like a good tutor — per the examples in §1, most concretely the hin_for_eng struggler (73 pauses, 24 back-skips, 5 restarts) producing something other than "full rate, every round." Watch `player_events` where `event_type='adaptation_plan'` for `applied:false` rows whose `plan` differs from the scripted defaults (`buildCount≠7 OR consolidateCount≠2 OR spacedRepCap≠12 OR insertBreather=true`) — that is the signal this re-soak is watching for.
