...
3. **THE NEAREST ANALOGOUS PAST FIX.** Search this handbook, then `git log` on the canon, then the estate: `/home/tomcassidy/command-surface/tools/estate-search "<your symptom>"`. Name the case you are reasoning from and say in one sentence why its shape is close and in one sentence where it differs.
...
And when Kai rules: the ruling goes into the canon and, if it is a new pattern, a new chapter is proposed here. A ruling that lands nowhere is a ruling that gets re-litigated.
...
`lang/` holds the per-language answers that a chapter deliberately refuses to state: `cym.md` (Welsh — the template, and why it is not a fix target), `gle.md` (Irish — the *try* ruling and the clash it opens), `fin.md`, `ita.md`, `ara.md`, `sin.md`, `zho.md`, `cat.md`. A language appendix is never evidence for another language.
