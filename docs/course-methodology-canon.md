...
**C0 — A live document tells agents to do the thing that caused the two-day outage.**
`scan-course.md` lines 1440–1469 instruct: delete the old `course_audio` record, then unlink, then regenerate. `CLAUDE.md` says deletion **never** precedes a verified replacement — "not even 'we'll regenerate right after'" — citing the fra_for_eng purge that left ~2,000 slots silent for two days. **Which stands?** (Recommend: make-before-break, and the scan-course passage is simply wrong.)
...
**A4 — Make-before-break: generate and verify the new asset before the old one is touched, always.** · **HARD** · `CLAUDE.md`; the fra_for_eng purge left ~2,000 slots silent for two days · (1) generate new audio, (2) verify each new clip is alive and correct-voiced, (3) swap links atomically, (4) only then delete the old clip · **Deletion never precedes a verified replacement — not even "we'll regenerate right after."** **See clash C0 — scan-course currently says the opposite.**

**A5 — Editing target_text mutates audio links, and the three tables behave three different ways.** · **HARD** · Verified live 2026-08-17 · `course_seeds`: **NO trigger at all** — the link keeps pointing at a clip speaking the OLD text, silently. `course_legos`: **NULLs** the presentation audio — loud, leaves the card silent until re-rendered. `course_practice_phrases`: **does NOT null, it RE-RESOLVES** — which is worse, because the link can silently land on a different clip, and the resolver matches on course/role/text but **not voice or language**, so it can silently swap voices · **A text-only fix never is one.**
