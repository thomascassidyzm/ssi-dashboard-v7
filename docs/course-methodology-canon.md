...
**C23 — Phase 8 still deletes presentation clips before regenerating them, in bulk.** *(New, 2026-08-27, surfaced while closing C0 — code, not a doc.)* `services/phases/phase8-audio-v13.cjs` ≈L4010–4058: when a LEGO's presentation template text changes, machine-origin `course_audio` rows are **deleted in batches of 200** and `presentation_audio_id` nulled, *then* the run regenerates. Human-origin rows are correctly protected by the precious-audio guard. But a run that dies between the delete and the render leaves those presentations gone — and the delete cascades away their revision history, flags and sign-offs, so a revert is impossible. This is the shape A4/O11 now forbid, compressed into one job rather than split across two agent actions. **Ruled doctrine and running code disagree. Not touched — this is a production render path and needs its own job.**
