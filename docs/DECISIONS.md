...
**Tom's builder test login is an `editor` scoped to one test course, `cat_for_gle` (2026-09-25, job #183, approved by Tom).** Login thomas.cassidy+poptybuilder@gmail.com holds a `dashboard_users` editor row with courses `["cat_for_gle"]` (the invite path's shape: a confirmed auth user plus that row, same as the Misko precedent), and a password on the auth user. The course came from `POST /api/courses/create` and is left at the product defaults: draft, visibility hidden, new/legacy app status not_available, and empty. The schema has no is_test column, so what keeps it off every learner catalogue is not_available.
