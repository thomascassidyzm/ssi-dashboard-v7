...
## 2026-09-13 — the splicer heals sub-5 ms blips inside a pause; cym_n pod-1 re-run: 3 more turns split, 36 go to the booth (job #599)

**Tom's ruling (21:40Z).** Teach the per-sentence splicer to ignore sub-millisecond blips inside an otherwise clean pause, then re-run on the 39 turns job #594 refused. Refuse-not-guess stays; the 100 ms / -35 dB floor stays; every other gate untouched.

**What was actually wrong.** Not the floor. ffmpeg's `silencedetect` only reports a run that stays below the floor for its whole `d`, so the photographed pause (60.7 ms + 0.136 ms blip + 81.8 ms) produced NO run at all — the existing 70 ms merge in `splice.py` never had anything to merge. The fix in `tools/pods/splice.py`: detect at `DETECT_MIN_S` = 10 ms resolution, heal runs separated by less than `BLIP_MERGE_S` = 5 ms of sound (`heal_blips`, pure), then apply the unchanged `MIN_SIL` floor to the healed run. Why 5 ms: the blip is a handful of samples; the shortest articulated sound (a tap ~20–30 ms, a plosive burst ≥5–10 ms) is above it, so no word is ever healed across. Anything that passed before still passes (a run already ≥100 ms is a superset of itself after healing). Four synthetic-waveform tests in `splice-sentence-clips.test.cjs`: clean pause, blipped pause, genuinely short pause, and a 30 ms sound between two short pauses that must NOT be healed; the blipped case fails on the pre-fix file and passes after.

**Re-run (orders scoped to the 39).** 4 accepted by the gates, 35 still refused (31 too_few_gaps — Aran and Catrin genuinely run the sentences together, cues found < cues in text; 2 margin_below_floor; 2 seam_not_silent, one of them NEW: order 22's healed 136 ms gap has a blip inside the 30 ms seam window at -32 dB, so the seam gate, untouched, refused the cut — the doctrine working). Of the 4, order 4 was refused BY HAND and not applied: healing merged a phantom "interior" gap (two adjacent runs inside the leading silence) into edge silence, the cue count then matched by coincidence, and whisper with Welsh forced showed piece 2 carrying both "Gobeithio… cei di ddiwrnod da." and "Wela i di wedyn." with piece 3 a 0.2 s breath. A bad split is worse than no split; the gates were left as ruled and the finding is logged here for Tom: the min-piece gate (0.35 s) cannot see a piece that is all breath. Applied live: orders 6 (the photographed turn), 67, 73 — six pieces cut from the speakers' own takes.

**Whisper on Welsh.** `verify-spliced-sentences.cjs` now maps `cym` → `cy`. With the language forced, ggml-small is usable as a listening leg on multi-word Welsh: it named the right sentence on every piece of orders 6/67/73 and caught the wrong cut on order 4. Still advisory (mutated consonants and proper names come out garbled); the hard checks are the proof.

**Booth marks.** `tools/pods/mark-splice-refusals-cym-n-2026-09-13.cjs` marks all 36 still-refused lines with the #590 lever, each to the voice that RECORDED it — 15 Aran, 21 Catrin Lliar — because a want naming a different voice would be a voice swap, which is casting, not this job. Reason text carries the splicer's numbers and asks for one take with a clear pause between sentences. Applied live; no serving pointer moved; one clip (order 41) kept its existing #590 want.

## 2026-09-13 — pod visibility and required_role become ONE atomic transition; the trigger refuses narrowing too (job #611)

**The race (GPT-6 Astra #608, after #605).** The role lever's compare-and-swap guarded only `required_role`, so a narrow that read (held, NULL), lost the CPU to a release, then wrote `WHERE required_role IS NULL` landed a role on a LIVE pod with a 200. And a body carrying both fields ran as two UPDATEs, so `{required_role: null, visibility: 'held'}` on a live pod cleared the role and then refused the hold — a partial success. Both break Tom's rule: a live pod is never narrowed, by anyone, ever.

**Decision.** `applyPodAccessChange` in `services/pod-visibility.cjs` is now the only write: one read, one judgement over both fields (the role judged against the visibility the row will have AFTER the request, so release-and-narrow in one body is refused as a live pod with a role), one UPDATE whose WHERE asserts BOTH the visibility and the role that were read; zero rows → re-read, re-judge: 200 (no-op) if the pod already matches the request, 409 otherwise — the rule text if the re-judged transition is forbidden, else "changed while in flight"; no partial writes because there is only one write. The route calls it once. `applyVisibilityChange` / `applyRequiredRoleChange` stay as thin names over it. Six new cases in `services/pod-visibility.test.cjs` — interleaved release-vs-narrow, combined partial-success, combined release-and-narrow, combined open-and-release in one write, same-role release, role-changed-under-release — red on the #605 module (seen: 200 and a cleared role), green after (44/44).

**The database guard.** `20260913_live_pod_never_narrowed_trigger.sql` extends `refuse_live_pod_hold()` to also raise on `NEW.visibility='live' AND OLD.required_role IS NULL AND NEW.required_role IS NOT NULL`, firing on both columns. Applied to production and proved with rolled-back UPDATEs: narrowing the Senedd/S4C pod raises, holding cym_n pod-1 raises, clearing a role on a live pod still passes. Rollback re-applies the #583 shape.

**Why better × simpler × cheaper.** One write path instead of two; the route lost 20 lines; no schema change; the DB guard costs one function edit and closes it for psql, sweeps and any second service.

## 2026-09-13 — the trigger also refuses re-scoping a live pod, role A → role B (job #614)

**The gap (GPT-6 Astra #612, after #611).** `refuse_live_pod_hold()` refused `(live, role NULL) → non-null`, but its condition required `OLD.required_role IS NULL`, so a LIVE pod changing from role A to role B still passed at the database while the route refused it. Tom's rule is broader: a live pod is never narrowed *or re-scoped*.

**Decision.** `database/changes/20260913_live_pod_never_rescoped_trigger.sql` rewrites the second branch as `NEW.visibility = 'live' AND NEW.required_role IS NOT NULL AND NEW.required_role IS DISTINCT FROM OLD.required_role`: on a pod that will be live, the only permitted role write is clearing it to NULL. Applied to production and proved with rolled-back UPDATEs on synthetic rows in one transaction (live A→B raises; live A→NULL passes; held A→B passes; live→held still raises; held/A→live/B in one UPDATE still raises; held/A→live/A passes; zero rows left). Transcript published: https://watson-1.tail4968cb.ts.net/d/f3977b67. The fake store in `services/pod-visibility.test.cjs` models only the compare-and-swap WHERE, not the trigger, so the DB proof lives in that transcript; the route's half of the same rule gained a live A→B case in the test file. Rollback re-applies the #611 shape. No service code changed; no deploy.

## 2026-09-14 — Senedd/S4C pod re-scened: 160 one-turn scenes become 57 scenes of ~10 sentences; the rule is now a tool (job #649)

**Commission (Aran, Tom "yes", 13:03 local).** The Senedd pod was ingested one scene per Senedd contribution — 160 scenes for 567 sentences, dozens of them one or two lines — and Aran asked for a hard rule of about ten sentences per scene.

**The rule, as `tools/pods/rescene-pod.cjs` now runs it.** Scenes are 8–12 sentences aiming at 10. A boundary falls where the speaker turn changes (turn = the `beat_label` contribution, else a run of one speaker). A question and its answer stay together: never a boundary directly after a sentence ending in a question mark, and `--no-break-after` names a question the punctuation missed. A turn longer than the band is split at a topic turn named by hand (`--split-after`), or, only when nothing else is legal, at the sentence boundary that best balances the scenes. Among all legal groupings the one whose sizes sit closest to 10 wins (least squared deviation over sentence boundaries — deterministic, reproducible); one sentence over or under the band is preferred to cutting a speaker mid-turn at an unnamed point, two is not. Every sentence is kept in its original order; only `scene_number` moves — ids, `sentence_number`, `global_order` and every clip pointer are asserted unchanged inside the transaction, before-state asserted per row, one `content_edit_events` row per pod under `serviceIdentity('rescene-pod')`. Nine tests in `rescene-pod.test.cjs` hold each clause. This is the default for future pods: run it on any pod that arrives one-scene-per-turn.

**Applied to production**, both rows identically (job #648's South twin already existed): `cym_n_for_eng:senedd-s4c-steve` and `cym_s_for_eng:senedd-s4c-steve`, 563 of 567 rows moved each, 160 → 57 scenes, sizes 7–13 (one 13: a three-line question bound to a ten-line answer; one 7: the closing regret answer, whose only topic turn is where it was cut). Thirteen named topic turns, all read from the transcript: after orders 20, 32, 56, 58, 107, 147, 157, 215, 318, 395, 416, 512, 551; order 50 (Carolyn's "when will this happen.") bound to its answer. No learner had progress on either pod (`learner_pod_state` empty for both), and ids did not change, so the content-change migration protocol has nothing to move. Visibility and `required_role` untouched.

Before (160 scenes): 4 1 1 2 2 19 15 2 2 2 16 2 2 7 2 1 4 3 2 2 5 2 3 10 1 8 1 4 5 9 5 10 6 8 2 8 1 1 1 2 3 1 1 7 2 2 4 2 4 9 6 2 1 5 2 5 2 1 2 6 1 4 2 2 6 1 1 1 1 1 2 2 1 1 2 1 11 2 1 1 5 3 1 2 1 1 1 1 2 3 9 1 4 5 2 1 12 1 5 1 4 4 3 1 1 5 1 1 1 1 2 2 5 8 11 2 1 7 6 4 2 10 2 1 6 1 1 1 2 2 7 2 3 5 4 3 5 4 1 3 1 1 11 4 11 1 1 6 2 1 3 5 2 1 3 2 13 4 4 1.
...
## 2026-09-19 — the canonical 231-sentence listening pod reaches all 40 courses still on the 142-set, as held text (job #271)

**The finding that set the shape.** Seventeen of the forty already carried a complete, verified 231-sentence pod on slug `unrecorded` — the August 2026 migration built and verified them and then stalled before promotion. They needed nothing. Of the remaining twenty-three, only **six** needed a language the estate has never rendered for this story (ben, guj, pan, sin, tam, urd); every other side of every other course already existed in a sibling course and was **copied**, because the canonical 231 is one story per language, not per course. Measured: the 231-set's structure — scene, sentence, global_order, speaker — is byte-identical across all 24 canonical courses, and exactly **five** English lines differ between them, the ones naming the language being learnt.

**So the five lines are the whole risk of a copy**, and `build-canonical-231-pod.cjs` refuses to guess them: it either derives the English side by substitution and asserts the line changed, or demands an explicit override from `draft-pod-known-side.cjs --mode=relabel`, which changes the language name and nothing else. A run aborts if an override is missing, blank, or identical to the line it replaces.

**Everything landed `visibility='held'` on slug `pod-1-231`**, which the player's resolver does not read; the live 142-sentence `pod-1` was never read and never written on any of the forty. No audio was rendered, no audio pointer was copied, and — deliberately, against CLAUDE.md's standing rule — **no audio pass was queued**, because a queued request is exactly what a later phase8 `/generate` would fulfil and Tom is choosing every pod voice himself.

**Provisional text says so.** Copied lines carry their source's own draft state rather than a uniform stamp. The six translated known sides land `target_text_draft = true` and go through `verify-pod-text.cjs` before anything may render from them. `target_text_draft` is the row's one flag and there is no `known_text_draft` column, so on an `eng_for_*` course it carries a draft that is actually on the known side; `target_text_review.draft_side` records which side the verdict was about.

**Three bugs fixed, each with a test that fails on the old code and passes on the new.** (1) `verify-pod-text.cjs` hardcoded `known_text` as the English, so on an `eng_for_*` course it handed the verifier a Bengali line labelled English and returned verdicts about nothing, silently — sides are now arguments (`pod-draft-sides.cjs`). (2) The expected-script table omitted ZWNJ/ZWJ, which are category Cf and which Sinhala uses for its conjuncts, so it failed 30 of 231 correct Sinhala lines as "not sinhala script". (3) A long run launched as a systemd unit wrote its intermediates to `$CS_SCRATCH`, which is swept when the launching session's job is marked done: the unit stayed up, exited 0, and five of six translations failed on a deleted directory.

## 2026-09-20 — the round map refreshes BY CONSTRUCTION on every lego write path (job #373)

**The decision: (A), a whole-view `REFRESH MATERIALIZED VIEW CONCURRENTLY`, coalesced, fired from the one middleware every content write already passes through.** Measured first, as Tom's commission asked: against the live database, with **92,439 view rows**, the refresh took **1,297 ms cold and 816/819 ms warm**. PostgreSQL has no per-course refresh of a materialised view — `REFRESH` is all-or-nothing — so the real fork was (A) whole-view CONCURRENTLY, (B) replace the matview with a table maintained per course inside the writing transaction, (C) a trigger on `course_legos` doing (B)'s work. At ~0.8 s for the whole estate, (A) wins on better × simpler × cheaper: one object, one definition, no schema migration, nothing new to maintain, and it cannot disagree with the nightly because the nightly now runs literally the same statement from the same module. (B) and (C) buy scoping the measurement says we do not need, and each costs a second definition of the view that can drift from the first — the exact failure class this job exists to close.

**Where it lives, and why there.** `services/shared/round-index-refresh.cjs` owns the statement, the connection and the coalescing; `services/shared/content-edit-gate.cjs` calls it on `res.on('finish')` for any surface flagged `legos: true` in `services/shared/content-write-surfaces.cjs`, on a 2xx only. Putting it in the gate rather than at the bottom of each handler is the whole point: a call every handler has to remember is the shape that left `afr_for_eng` nine rounds short of its own content for weeks, silently. **Forgetting the flag is now a test failure** — `content-write-surfaces.test.cjs` re-derives the `course_legos` writers from the route sources and fails in both directions, the way it already did for the identity manifest.

**Fire-and-log, not awaited by the route.** A second of refresh on the end of every seed submission would add 90 seconds to a 90-seed build and make a hot route's latency depend on the size of the whole estate. The caller gets its response immediately; the promise is returned for the callers that do want to wait (a `tools/` sweep about to exit, and the tests). Bursts coalesce: 90 submissions cost one refresh, and a write landing mid-refresh schedules exactly one more, so the view is always refreshed after the *last* write. A failed refresh never rejects into the caller — it resolves `{ ok: false }` and logs loudly, because failing a content write that already succeeded would be worse than a stale map.

**The nightly stays, as the backstop Tom said it was.** `tools/round-index/nightly.cjs` and `tools/refresh-round-index.cjs` are unchanged in behaviour; the tool now imports `REFRESH_SQL` and `databaseUrl` from the shared module so there is one mechanism and not two. Its job after this change is to catch the case where a write path was added without the refresh — which the drift test now catches earlier.

**What was deliberately not wired.** The phrase v3 door (`routes/phrases-v3.cjs`, `lib/phrase-generation.cjs`) writes phrases and never touches `course_legos`, so it moves no round and pays nothing. `tools/basket-rework.cjs` updates a lego's `known_text` and audio ids only — not `seed_number`, `lego_index`, `lego_id` or `is_new` — so the view's contents are unchanged by it. `tools/gle-round-realign-2026-09-10.cjs` writes learner progress columns, not legos. The two live harnesses that create and destroy legos on throwaway courses (`tools/redo-snapshot-live-harness.cjs`, `tools/audio-regen-probe/harness.cjs`) are not wired: they clean up after themselves and their courses never ship.

## 2026-09-20 — the four gaps in the by-construction refresh, closed (job #384)

**The refresh now fires on ANY response from a lego-writing surface, not only a 2xx.** A cross-family verification found the hole and reading `seed-complete.cjs` confirmed it: `/api/lego`, `/api/batch` and `/api/seed/complete` all commit the `course_legos` row and THEN write its phrases, so a phrase failure answers 500 — and a ZUT check answers 400 — with legos already in the table and the map left short of content that had landed. The considered alternative was a `req.contentEdit.legosWritten` signal set after each successful lego upsert; it was rejected as the more expensive of the two, because it means threading a flag through seven route files and every `lib/` helper that writes legos (`redo-snapshot.cjs`'s `restoreSnapshot()` has no `req` at all) and it drifts the moment somebody adds the eighth — the per-handler shape this design exists to avoid. A wasted refresh on a failed write costs one coalesced ~0.8 s statement off the caller's path; a missed one costs a silently short course.

**Three smaller defects, each now with a test that fails on the old code and passes on the new.** (1) `LEGO_WRITERS_VIA_HELPERS` in the drift test only excused an extra flag and never required one, so deleting `legos: true` from `POST /api/build/redo-undo/:courseCode` — the one route the list exists to protect, and the one the route scanner cannot see — passed silently; a key there must now be flagged. (2) `requestRoundIndexRefresh` armed its timer without looking at `inFlight`, so a request arriving mid-run started a second concurrent `REFRESH` on a second connection; it now only marks pending, and `runPending`'s tail re-arms as it already did. (3) The timer was unconditionally unref'd, so a script whose last act is `await requestRoundIndexRefresh(..., {immediate:true})` could exit before the refresh ran and print success over a stale map — `tools/gle-lego-reorder-2026-09-09.cjs` and `tools/course-optimization/duplicate-course-teaching-layer-2026-09-02.cjs` were both in that shape. An immediate request now holds the loop open, and both tools flush before exit.
## 2026-09-20 — the Voice Lab: our own clones are always findable, three ranks per gender, and deleting a clone never touches audio (job #383)

**Items 1 and 2 were one bug, and it was an ownership bug rather than a filter bug.** Tom could not cast Aran's clone as the English guide, and `owned` was undefined on every candidate in the estate — measured live against the production registry that evening. The 2026-08-29 fix sorted owned CATALOGUE voices ahead of the 80-item cap and still works, but it could say nothing about a clone that also has a `voices` row: `!voiceById.has(id)` drops such a voice from the owned path, and the registered path never carried Cartesia's `owner` flag at all. By 2026-09-20 all four English clones had rows, so all four were invisible as ours — Aran's sat at position 18 of 28 in the guide list under the name `aran_english_003`. **Ownership is now read from the catalogue once (`ownedCloneIds`) and stamped on every candidate whichever path built it, then `ownedFirst` sorts owned to the top BEFORE the cap** — which is what makes "always findable" true rather than intended. The badge on the row (`our clone`) is what makes it visible rather than merely true.

**The one filter ownership does NOT beat is `is_active`,** and that is deliberate: retirement is how a clone is deleted. `dropRetired` keeps a deactivated clone from walking back in through Cartesia's own catalogue — without it, "delete" would have put the voice back at the TOP of the list.

**Deleting a clone has two modes and the clip count picks, never the operator.** A voice nothing has ever been rendered with is REMOVED (clone deleted at Cartesia, then the row). A voice that already speaks is RETIRED: `is_active = false` and nothing else — no clip deleted, none re-pointed, none re-rendered, which is Tom's hard rule of the same evening ("we also must NOT re-render any old audio"). A hard remove is refused with 409 the moment a clip exists, even when asked for by hand, because the `voices` row is the only record of who is speaking in those 790 places. A voice still cast into a slot is refused as it always was. `GET /voices/:voiceId/removal` is a read-only preflight so the screen states the count in plain words before the button, and the sentence the human reads is the same fact the route enforces. On the estate today: `tom_001` (790 clips, cast) is refused; `Tom_002`, `Tom_003` and the two Italian sample clones have zero clips and delete outright.

**Three ranks per gender, two for the guide, and completeness did not move.** `REQUIRED_RANKS` 2 → 3 for the phrase slots; `COMPLETE_RANKS` stays 1, so a language is still complete on one male and one female primary and the four extra slots are quiet capacity for the pod work. The guide keeps two ranks under its own `GUIDE_RANKS`: a guide is one voice speaking to a learner, and multiplying its backups would add empty rows for a decision nobody is making. No migration: `voice_language_roles` already checks `rank <= 5`. The short slot labels became `F·pri / F·bak / F·bak2` because the old first-three-letters rule drew two buttons reading `F·bac` on the same row.

**Community builders start on the course journey, with three cards, and never go via the overview (2026-09-25, job #189, Tom and Aran; preview only until Tom promotes).** An `editor` lands on `/production/:code/journey`, and the route guard sends every overview link there. The top of the page is Tom's three cards, "I want to record / proofread / build some stuff", and each goes straight to where that work is done. Record opens the builder's own booth (`/r/:voiceId?course=`) when the casting names a voice for them on this course. Otherwise it opens the cast (`pods?cast=1`). Proofread opens the course read in order, with the pod drafts as a second link. Build opens the builder. The pick is remembered in localStorage per email: `dashboard_users` has no preferences column, and one UI choice is not worth a schema change on the shared database. The "Build the full audio" step is greyed on a human-voice course (the human-voice-courses rule, or both target slots human), as Tom ruled. In code that step is the human-recording stitcher, not TTS. **The Admin tab's whole territory is admin-only**, read off the nav declaration (`isAdminOnlyRoute`, with `/builds` open to everyone). The machine switcher and Deploy render for admins only. The Voice Lab is admin-only at its router's edge except its consent doors, and the cast roster offers SSi staff only to staff. **A cast recorder can never edit seed, LEGO or phrase text:** the content-edit gate refuses `role: recorder` and any login without a grant on the course, on every manifest surface. Pod lines stay editable from the booth, own lines only, as before. **Sending the link is the editor's own email** (a pre-addressed mailto). Popty has no mail service of its own, and the only Resend key on the box belongs to Hexagon, which the borrowed-credential rule forbids for sending.
