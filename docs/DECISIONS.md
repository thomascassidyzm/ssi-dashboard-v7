...
**2026-09-25 — Recordist emails out of the public key's reach; Basket Lab admin-only (Tom: "fix that" / "Lock it, shouldn't be public"; job #195·I).** Every `email`/`assignedEmail` key in `courses.voice_config` and `language_recording_policy.voices` now lives in `public.recordist_emails` (service-role only: anon/authenticated revoked, RLS on, no policy), moved there on every write by a trigger (`ops/sql/20260925-recordist-emails-{vault,scrub}.sql`), so no writer can put one back. Popty's service-role clients hydrate them back on read (`services/shared/recordist-email-vault.cjs`), so booth, casting and cast screens are unchanged; the learner app reads no email key on main or release/2026-09-25, so it needed no change. Basket Lab sits behind `requireAdmin` on its API mount; the dashboard iframe enters with a requireAdmin-minted ticket that buys a 12h lab cookie (`services/shared/basket-lab-gate.cjs`). #189's Voice Lab admin lock and staff-roster filter were cherry-picked to main with it.
