...
const rows = require('./recomposed4.json')
...
async function main() {
  ...
  const dir = path.join(__dirname, 'last4'); fs.mkdirSync(dir, { recursive: true })
  const log = []
  ...
  if (!APPLY) {
    ...
    fs.writeFileSync(path.join(__dirname, 'last4-dryrun.json'), JSON.stringify(log.map(({ _ship, ...r }) => r), null, 1))
    await db.end(); return
  }
  ...
  fs.writeFileSync(path.join(__dirname, 'last4-applied.json'), JSON.stringify(log.map(({ _ship, ...r }) => r), null, 1))
  await db.end()
}
