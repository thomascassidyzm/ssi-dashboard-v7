...
**Landing line.** Three commits — `47d32f00a` (attribution), `49b0fbf8f` (flagger rule),
`05e411a66` (this report) — are on branch `fix/pod1-speaker-attribution-2026-08-24` and
were **merged to `main`**, which now sits at `05e411a66` on origin. **Not deployed:** the
`ssi-dashboard-v7-clean-prod` checkout that runs the services is still at `57ed8fd17`, and I
verified live on `localhost:3470` that it serves the new attribution (data) but the old,
un-addressed finding text (code).
