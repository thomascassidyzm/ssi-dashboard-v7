...
live in the DB against `cym_n_for_eng` (Northern Welsh) pod-1. Then the session stopped and
...
|---|---|---|
| Cast — assign real people to voice slots | `src/components/PodCastPanel.vue`, `services/voice-engine/pods-cast.cjs` | `listening_pods.speakers` for `cym_n_for_eng:pod-1` holds a real 5-slot cast (HUMAN_F1/F2/F3/M1/M2 → Sarah, Anna, Assistant, Neighbour, Tourist, etc.) |
...
1. **`cym_n_for_eng` is 20% recorded (28/142 lines), `cym_s_for_eng` is 0%.** This is pure
   ...
   roughly 2.5 hours got 28 lines from one voice-slot pair, so full pod-1 coverage for one
...
3. **The Welsh script predates the ellipsis-pause convention.** `insert-ellipsis-seams.cjs`
   ...
   existing pod-1 lines are already short, single-clause sentences split by full stops).
   *Effort if wanted before recording resumes: ~10 minutes to dry-run the tool against both
   Welsh courses at pod-1's C=8 ceiling and eyeball the diff — optional, not blocking.*
...
- **Two Welsh courses exist and are cast, live in Supabase**: `cym_n_for_eng` (Northern/
  Gogledd) and `cym_s_for_eng` (Southern/De). Both are pod-1 only — 142 lines each, the same
...
- **Why human recording matters here specifically**: pod-1's own pipeline-readiness check
  ...
  audio at all, for pod-1 and for any future pod.
...
**Can Aran start recording Welsh today? Yes, pod-1, right now — he's already proven it works.**
