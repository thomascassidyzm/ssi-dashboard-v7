...
| **Pod Lab** | `/admin/configs/pods` | **WRITES, but never config** — `PATCH /api/pod-fine-map`, `POST /api/pod-cast-voices`, `POST /api/pod-voice-approval` | `atom_map_fine`, `listening_pods.speakers`, voice approvals | **one course.** ~~none of the three is immediate~~ **CORRECTED 2026-09-01:** `atom_map_fine` is **not** a draft column and the fine-map write **is** immediate — the learner's Drill reads it live off `listening_pod_sentences` on every fetch (`useListeningPods.ts:179` → `buildFusionGroups`; the `fusionDrill.ts:38` kill switch suppresses sub-sentence audio only, and says "text chunking and glosses stay fully intact"). Casting and approval are deferred. | yes |
