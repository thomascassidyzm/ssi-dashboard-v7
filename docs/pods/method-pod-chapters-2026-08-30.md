# The Method Pod in Italian — the chapter cut

> **PRIVATE. Unlisted, Italian only, audience of two — Tom and Aran.** A second, parallel artefact beside `method-pod-full-2026-08-30.md` (the 43-scene cut, on `main` at `b7f227ba3`), which stays exactly as it is as the control arm. Nothing here executes: no row is written, no pod is edited, no audio is queued or rendered, no course is touched, nothing is deployed. It is a document to be read and heard, so that the two forms can be compared side by side and one of them chosen.
...
## 0. The comparison — chapters against scenes, cold
...
- **Callbacks (four, all real reach-backs the ear can catch, none a thematic echo):** Chapter 8 reaches back to Chapter 2's *"accettala lo stesso"* — the anecdotal-evidence line is used as a tool sixty turns later and its ownership is argued; **Chapter 9** rebuilds its lost "third thing" from three earlier chapters by name — the building in the field (Ch. 1), the forgetting curve (Ch. 2) and the cliff (Ch. 6); **Chapter 11** quotes Tom's atoms line back to him from Chapter 10, verbatim, and puts it to work on money; **Chapter 12** picks Chapter 7's *"la mettiamo in pausa"* back up and refuses it — this one does not go on pause.
- **The flagged digression-and-return:** Chapter 5 — Aran flags the garden as a detour, Tom runs the clock across it, the detour wanders through three turns and returns on its own marker, and the return lands because forty words of garden have gone by, not twelve.
- **Reformulations (three):** Chapter 5 — *"quindi quello che stai dicendo è"*, accepted and sharpened; Chapter 4 — *"so what you're saying is there are no rules"*, refused and corrected; Chapter 6 — *"films with subtitles"*, refused in one word and laid side by side.
- **Shapes the scene form could not hold at all:** the co-owned line as a callback rather than a cousin; the thread *banked in one chapter and recovered in the next* (N17 both halves, across a session break, as it happens in life); the parked clash and the standing clash as two ends of one argument rather than two unrelated scenes.
...
## 1. The chapter map — twelve points, and what each absorbs

Delivery order is still derived, not authored; but a chapter cut has a *reading* order because the callbacks only work forwards, and that order is the one below. Chapters 1–8 are session one; the dog ends it. Chapters 9–12 are session two.

| # | Title | The one point | Absorbs (scene #) | Shape budget | Long-arc move |
|---|---|---|---|---|---|
...
| 8 | *Uno a uno* | What a group does to a week — and why it is still worth doing | 24, 9, 38, 13, 12, 14, 18 | O1 the bare no · the audit inside the glow · the listener names it · the co-owned line (m20) · metaphor handover (m16) · N17 the bank (m19) · N7 arrangement · O9 second no | **callback → Ch. 2** ("accettala lo stesso") · the session ends on the dog |
| 9 | *La terza cosa* | Why the evening class fails — three things, one of them lost and rebuilt | 41 | N17 the recovery · not-knowing (m22) · joint construction (m2) | **callback → Ch. 1, Ch. 2, Ch. 6** (the three things rebuilt from where they were said) |
| 10 | *Due giorni* | What two days a year apart actually show | 20, 35, 11, 19, 36, 39 | the stacked commission · story → matched story · the mirrored tease (m17) · O4 read-back corrected · O7 disclosure acknowledged · O6 own-fault | plants the atoms line (for Ch. 11) |
| 11 | *I soldi* | Value is invented, and then it is never lost — only moved | 15, 34, 33 | the puppet show (m15) · the callback (m8) · the world breaks in (m9) | **callback → Ch. 10** (the atoms, verbatim) |
| 12 | *Efficienza e velocità* | What we are building for — and the disagreement stands | 42, 37, 43 | O8 standing clash · N8 recommendation on guarantee · N1 close · the resisted goodbye | **callback → Ch. 7** (*in pausa* refused) |

Every scene number 1–43 appears exactly once in the "absorbs" column. Scenes 13, 20 and 41 are the three that were rewritten heavily enough to be named in the report; everything else keeps its lines where the chapter allowed and re-voices them where it did not.
